...
app.post('/gauges/update', (req, res) => {
    ...
    dashboard.panels.forEach(panel => {
        if (panel.fieldConfig && panel.fieldConfig.defaults) {
            ...
            if (updates[title]) {
                ...
                if (newMin !== undefined) panel.fieldConfig.defaults.min = newMin;
                if (newMax !== undefined) panel.fieldConfig.defaults.max = newMax;

                // Update thresholds based on new max values
                const thresholds = panel.fieldConfig.defaults.thresholds.steps;
                if (thresholds) {
                    thresholds[1].value = newMax * 0.25;
                    thresholds[2].value = newMax * 0.50;
                    thresholds[3].value = newMax * 0.75;
                    thresholds[4] = { color: "red", value: newMax };
                }
            }
        }
    });
    ...
    res.json({ message: 'Gauges and thresholds updated successfully.', success: true });
});
