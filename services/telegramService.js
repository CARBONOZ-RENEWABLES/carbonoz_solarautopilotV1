...
function checkSmartPowerConditions(config, systemState) {
  const isEnabled = config.smartPowerConditions?.enabled || false;
  const rules = config.smartPowerConditions?.rules || [];
  const hasCustomRules = rules.length > 0;
  
  // FIXED: If smart power conditions are disabled completely
  if (!isEnabled) {
    return { 
      allow: true, 
      reason: 'Smart power conditions are disabled',
      details: { 
  ...
  // FIXED: If enabled but no custom rules defined - this is OPTIONAL, not a failure
  if (!hasCustomRules) {
    return { 
      allow: true, 
      reason: 'Smart power conditions enabled but no custom rules defined (optional feature)',
      details: { 
        smartPowerEnabled: true,
        hasCustomRules: false,
        rulesCount: 0,
        optional: true,
        userDefined: true,
        note: 'You can add custom power rules in the settings to enable advanced power-based charging logic'
      }
    };
  }
  ...
  // No rules passed - this is an actual failure since user has defined rules
  ...
  return {
    allow: false,
    reason: `Custom power rules not satisfied (${results.filter(r => !r.passed).length} of ${results.length} rules failed)`,
    priority: 'low',
    details: {
      smartPowerEnabled: true,
      hasCustomRules: true,
      ...
      userDefined: true,
      systemState: {
        battery_soc: getCurrentSystemValue('battery_soc', systemState),
        pv_power: getCurrentSystemValue('pv_power', systemState),
        load: getCurrentSystemValue('load', systemState),
        grid_power: getCurrentSystemValue('grid_power', systemState),
        battery_power: getCurrentSystemValue('battery_power', systemState),
        grid_voltage: getCurrentSystemValue('grid_voltage', systemState)
      }
    }
  };
}
...
function addUserSmartPowerRule(config, newRule) {
  ...
  // FIXED: Auto-enable smart power conditions when first rule is added
  if (config.smartPowerConditions.rules.length === 1) {
    config.smartPowerConditions.enabled = true;
    console.log('🔧 Auto-enabled smart power conditions (first rule added)');
  }
...
function removeUserSmartPowerRule(config, ruleId) {
  ...
  if (index !== -1) {
    ...
    // FIXED: Auto-disable smart power conditions if no rules remain
    if (config.smartPowerConditions.rules.length === 0) {
      config.smartPowerConditions.enabled = false;
      console.log('🔧 Auto-disabled smart power conditions (no rules remaining)');
    }
